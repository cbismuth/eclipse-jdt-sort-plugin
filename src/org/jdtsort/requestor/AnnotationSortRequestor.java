...
import org.jdtsort.visitor.AnnotationVisitor;

import static org.jdtsort.Logger.log;

public class AnnotationSortRequestor extends AbstractASTSortRequestor {
    private final IASTNodeSorter<Annotation> sorter;

    public AnnotationSortRequestor(IASTNodeSorter<Annotation> sorter) {
        this.sorter = sorter;
    }
    ...
    public void acceptAST(ICompilationUnit unit, CompilationUnit root) {
        ...
        try {
            ...
            if (!types.isEmpty()) {
                ((AbstractTypeDeclaration) types.get(0)).accept(new AnnotationVisitor(root, sorter));
            }
